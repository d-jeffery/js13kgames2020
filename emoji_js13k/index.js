...
function recurrentLogStorageSize() {
  ...
  function logStorageSize() {
    const storageSize = storage.interface.size();
    const pct = storageSize ? storageSize / limit * 100 : 0;
    if (lastStoragePct !== pct) {
      const color = pct <= 100 ? chalk.green : chalk.red;
...
storage.init(app.get('storage')).then(() => {
  ...
  if (typeof sandbox.module.exports == 'function') {
    ...
    app.use(parser.urlencoded({extended: true}))
        .use(parser.json());
    for (const route in sandbox.module.exports) {
      if (route == 'io') {
  ...
  server.listen(app.get('port'), () => {
    ...
    createZip()
        .then(()=> {
          if (isDevelopment) {
            const pct = packageSize / limit * 100;
            const color = pct <= 100 ? chalk.green : chalk.red;
